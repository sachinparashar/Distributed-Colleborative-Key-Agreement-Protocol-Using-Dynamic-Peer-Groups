...
import java.sql.*;
import java.io.*;
public class AddImpl extends UnicastRemoteObject implements AddInter
{
	...
	public ArrayList getMember(String query) throws RemoteException
	{
	...
	public ArrayList getChilds(String query) throws RemoteException
	{
		ArrayList list=new ArrayList();
		try
		{
...
public ArrayList getMemberNames(String query) throws RemoteException
	{
		ArrayList list=new ArrayList();
		try
		{
...
public void writeFile(String content, String filename, String desname, String username,java.util.Date date) throws RemoteException
	{
...
public void writeFileAll(String content, String filename, java.util.Date date, String username) throws RemoteException
   ...
   try
   {
	...
	File f=new File("");
String s="select memname from tree where s1 is null or s2 is null";
	...
	public int getMembers(String query) throws RemoteException
	{
		...
		try
		{
			...
			if(rs.next())
			{
				...
				ConnectDatabase.st1.executeUpdate("INSERT INTO tree (members) values ("+l+")");
				int r=2*member+2;

				ConnectDatabase.st1.executeUpdate("INSERT INTO tree (members) values ("+r+")");				
				ResultSet rs1=ConnectDatabase.st1.executeQuery("select memname,pass from tree where members="+member);
				if(rs1.next())
				{

					ConnectDatabase.st1.executeUpdate("update tree set memname='"+rs1.getString(1)+"',pass='"+rs1.getString(2)+"' where members="+l);
					ConnectDatabase.st1.executeUpdate("update tree set s1="+l+",s2="+r+",memname='  ',pass='  ' where members="+member);
				}
			...
			{
				int t=ConnectDatabase.st1.executeUpdate("INSERT INTO tree (members) values (0)");
			}
		}
		catch (Exception e)
		{
...
public String getRequest123(String qqq1) throws RemoteException
{
	String strr1=null;
	String str123=null;
	//System.out.println("qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq"+qqq);
...
public void deleteMember(String membername) throws RemoteException
{
	String member="";
	String password="";
	String memname="";
	...
	try
	{
		ResultSet rs=ConnectDatabase.st1.executeQuery("select members from tree where memname='"+membername+"'");
		if(rs.next())
		{
			member=rs.getString(1);
		}
		rs.close();
		ResultSet rs1=ConnectDatabase.st1.executeQuery("select members,s1,s2 from tree where s1='"+member+"' or s2='"+member+"'");
		if(rs1.next())
		{
			...
			if(!s1.equalsIgnoreCase(member))
				query="select memname,pass from tree where members='"+s1+"'";
			else
				query="select memname,pass from tree where members='"+s2+"'";
			...
			int a=ConnectDatabase.st1.executeUpdate("delete from tree where members='"+s1+"'");
			int b=ConnectDatabase.st1.executeUpdate("delete from tree where members='"+s2+"'");
			int t=ConnectDatabase.st1.executeUpdate("update tree set s1=null,s2=null,memname='"+memname+"',pass='"+password+"' where members='"+mem+"'");
